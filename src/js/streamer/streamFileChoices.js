...
export function renderFileChoices(data, placeholderContainer, chatWindow) {
    ...
    header.className = "text-xs font-bold text-slate-400 normal-case tracking-normal mb-2 text-left flex items-center gap-1.5 border-b border-slate-850 pb-2";
    header.innerHTML = `
            ...
            <div class="flex items-center justify-between p-3.5 cursor-pointer bg-slate-950/40 hover:bg-slate-950/80 transition-colors duration-150" onclick="window.toggleFileAccordion(this)">
                <div class="flex items-center gap-3 truncate flex-1 min-w-0">
                    ...
                    <div class="truncate flex-1 min-w-0 text-left">
                        <div class="text-xs font-bold text-slate-200 truncate tracking-normal">${file.generated_title}</div>
                        <div class="text-xs text-slate-500 truncate">${file.original_name}</div>
                    </div>
                </div>
                ...
                <div class="p-4 border-t border-slate-950 flex flex-col gap-3">
                    ${isImage
                        ...
                        : `<div class="bg-slate-950/95 border border-slate-850 rounded-lg p-3 text-xs font-mono text-slate-300 max-h-60 overflow-y-auto whitespace-pre-wrap text-left relative min-h-[60px]">
                             ...
                             <pre class="document-lazy-load hidden text-xs leading-relaxed select-text" data-file="${file.physical_name}" data-loaded="false"></pre>
                           </div>`}
